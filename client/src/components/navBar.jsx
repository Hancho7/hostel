  ...
  return (
    <div className="p-4 bg-[#f8f9fa] md:flex md:flex-row md:items-center md:justify-between">
      <div className="flex items-center justify-between">
        <img src={logo} className="w-10 h-10 rounded-full hover:cursor-pointer" onClick={()=> Navigate("/")} alt="logo" />
      ...
      {nav ? (
        <div className="flex flex-col items-center gap-y-4 md:flex-row md:gap-x-20 duration-700">
          <Link className="h-full" to="/">Home</Link>
          ...
          {user ? (
            ...
            <>
              {
                role === "user" ? (
                  <button className="bg-[#b03434] w-20 h-8 font-semibold text-white rounded hover:bg-[#7a95c7]" onClick={() => dispatch(logout())}>Logout</button>
                ) : role === "manager" ? (
                  <>
